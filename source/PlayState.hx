...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'helipad': //Week Desp Vegeth
				var sky:BGSprite = new BGSprite('desp/helipad/sky', -600, -200, 0.7, 0.7);
				add(sky);

				var townBack:BGSprite = new BGSprite('desp/helipad/townBack', -650, -200, 0.7, 0.7);
				townBack.updateHitbox();
				add(townBack);
				if(!ClientPrefs.lowQuality) {
					var stageLight:BGSprite = new BGSprite('desp/helipad/stage_light', -125, -100, 0.9, 0.9);
					stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
					stageLight.updateHitbox();
					add(stageLight);
					var stageLight:BGSprite = new BGSprite('desp/helipad/stage_light', 1225, -100, 0.9, 0.9);
					stageLight.setGraphicSize(Std.int(stageLight.width * 1.1));
					stageLight.updateHitbox();
					stageLight.flipX = true;
					add(stageLight);

					var stageCurtains:BGSprite = new BGSprite('desp/helipad/stagecurtains', -500, -300, 1.3, 1.3);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.9));
					stageCurtains.updateHitbox();
					add(stageCurtains);
				}
